...
The Split: The CMS is a Next.js App Router project. The Live Frontend is a separate Vite React SPA located in the /frontend folder.
...
Data Flow (Backend): The Next.js API routes must write JSON files directly into the Vite public folder to bypass OS symlink blocks.
Example Path: path.join(process.cwd(), 'frontend', 'public', 'data', 'filename.json')
...
Example Fetch: fetch('/data/filename.json?t=' + new Date().getTime())

🎨 2. UI & CSS STANDARDS (STRICT)

Styling: Strictly Vanilla CSS Modules. No Tailwind. No default browser styling.
Inputs: Must be transparent (background: transparent;) with a subtle stroke. Prevent webkit autofill styling using the 5000s transition hack. Text color must contrast correctly.
...
Layouts: * Rows must use display: flex; flex-direction: row; align-items: center; gap: 8px; to keep inputs and remove buttons strictly on the same line.
