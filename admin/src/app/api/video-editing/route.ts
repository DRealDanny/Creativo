...
export async function GET() {
  try {
    ...
    console.error('Error reading video-editing data:', error);
...
export async function POST(request: Request) {
  try {
    ...
    if (projectData[existingIndex].gridPreview && projectData[existingIndex].gridPreview.gridTitle) {
        ...
        projectData[existingIndex].slug = 'video-editing-project';
    }
    ...
    console.error('Error writing video-editing data:', error);
    return NextResponse.json(
      { error: 'Failed to update video-editing data' },
      { status: 500 }
    );
  }
}
